...
// OAuth2 Контексты
import { OAuth2AuthProvider } from './contexts/OAuth2AuthContext';
...
const DesktopNavbarOAuth2 = React.lazy(() => import('./components/Desktop/DesktopNavbarOAuth2'));
...
const MobileNavbarOAuth2 = React.lazy(() => import('./components/Mobile/MobileNavbarOAuth2'));
...
const AppContent = () => {
  const { deviceMode, isMobile } = useDevice();

  // Определяем какие компоненты использовать
  const Navbar = isMobile ? MobileNavbarOAuth2 : DesktopNavbarOAuth2;
      ...
      <BrowserRouter>
        ...
        <Routes>
          ...
          <Route path="/profile" element={
            <Suspense fallback={<LoadingSpinner size="large" text="Загрузка профиля..." />}>
              <div className="p-8 text-center">
                <h1 className="text-2xl font-bold mb-4">Профиль пользователя</h1>
                <p className="text-gray-600">
                  {isMobile ? 'Мобильная' : 'Десктопная'} версия профиля в разработке
                </p>
              </div>
            </Suspense>
          } />
          <Route path="/orders" element={
            <div className="p-8 text-center">
              ...
              <p className="text-gray-600">
                {isMobile ? 'Мобильная' : 'Десктопная'} версия заказов в разработке
              </p>
            </div>
          } />
          ...
          <Route path="/admin/users" element={
            <div className="p-8 text-center">
              ...
              <p className="text-gray-600">
                {isMobile ? 'Мобильная' : 'Десктопная'} версия управления пользователями в разработке
              </p>
            </div>
          } />
